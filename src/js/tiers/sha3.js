 ...
 */
const _sha3Methods = ((root, undefined) => {
  ...
  var createOutputMethod = (bits, padding, outputType) => (message) =>
    new Keccak(bits, padding, bits).update(message)[outputType]();

  var createShakeOutputMethod =
    (bits, padding, outputType) => (message, outputBits) =>
      new Keccak(bits, padding, outputBits).update(message)[outputType]();
  ...
  var createShakeMethod = (bits, padding) => {
    ...
    method.update = (message, outputBits) =>
      method.create(outputBits).update(message);
    for (var i = 0; i < OUTPUT_TYPES.length; ++i) {
      var type = OUTPUT_TYPES[i];
