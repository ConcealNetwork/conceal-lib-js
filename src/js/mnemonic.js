...
var crc32 = (() => {
  ...
  crc32.Utf8Encode = (string) => unescape(encodeURIComponent(string));
...
(() => {
  for (const ws in mn_words) {
    if (!Object.hasOwn(mn_words, ws)) continue;
    if (mn_words[ws].prefix_len === 0) continue;
    mn_words[ws].trunc_words = [];
    for (let j = 0; j < mn_words[ws].words.length; j++) {
      mn_words[ws].trunc_words.push(
        mn_words[ws].words[j].slice(0, mn_words[ws].prefix_len),
      );
    }
  }
})();
